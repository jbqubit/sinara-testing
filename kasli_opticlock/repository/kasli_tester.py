...
class KasliTester(EnvExperiment):
    ...
    def init_urukul(self, cpld):
        self.core.break_realtime()
        #sta = cpld.sta_read()
        #print(artiq.coredevice.urukul.urukul_sta_proto_rev(sta))
    ...
    @kernel
    def set_zotino_voltages2(self, zotino):
        self.core.break_realtime()
        delay(300*us)
        zotino.init()
        delay(300*us)
        while True:
            for i in range(100):
                for ch in range(8):
                    zotino.write_dac(ch+8, ch+1/100*i)
                    delay(100*us)
                delay(60*us)
                zotino.load()

    def test_zotinos2(self):
        for card_n, (card_name, card_dev) in enumerate(self.zotinos):
            self.set_zotino_voltages2(card_dev)
